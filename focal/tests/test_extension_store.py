from focal.persisters.extension_store import *
import pytest
import numpy as np
import pandas as pd
...
test_tuples = ((test_df, '.csv', df_comparison),
...
def test_store(object_to_test,
    ...
    assert res_equl == True, f"For {extension_to_test=}, saved and retrieved are not the same:" \
                             f" \n{retrieved=}" \
                             f"\n{object_to_test=}"
